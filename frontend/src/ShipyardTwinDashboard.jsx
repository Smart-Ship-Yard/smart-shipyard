 ...
 *          · 좌현 옆면에 딱 붙임  →  참값 좌우 = -0.07 (= -SHIP_REAL_BEAM_M/2)
 *          · 우현 옆면에 딱 붙임  →  참값 좌우 = +0.07
 *          · 앞뒤 중앙            →  참값 앞뒤 = 0
 ...
 *     뱃머리 좌현  (+0.35, -0.03)   (+0.331, +0.002)  (-0.019, +0.032)
 *     선미  우현  (-0.35, +0.03)   (-0.399, +0.022)  (-0.049, -0.008)
 *
 *   앞뒤: 두 불 모두 **음수**(선미 쪽으로 밀림), 평균 -0.034 → 계통 편향으로 보고
 *         +0.034 를 넣는다.
 *   좌우: -0.008 과 +0.032 로 **부호가 반대**다. 계통 편향이라는 근거가 없다.
 *         평균(+0.012)을 넣으면 잡음에 맞추는 꼴이라 **0 으로 둔다.**
 *
...
const PING_OFFSET_BEAM_M = 0.0;       // + 가 우현 쪽 (실측 결과 편향 없음 — 아래)
