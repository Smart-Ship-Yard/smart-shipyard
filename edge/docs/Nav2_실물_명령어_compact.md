...
> `전체_실행_명령어_요약본_매핑부터_자율주행까지.md` 를 볼 것.
...
3. **사람은 로봇 앞 0.5 m 밖에서 가로지를 것. 뒤로는 다가가지 말 것.**
   ...
   정후방 150도도 안 보인다(라이다가 보는 것은 전방 210도뿐).
   장애물을 만나면 **15 cm 후진**하는데 그때도 뒤를 못 본다.
   보이기만 하면 10 cm 안에 서므로 문제는 제동이 아니라 **보이느냐**다.
   상세 근거와 실측: 설치가이드 6-3절.

4. **★ 시연 전, 서버 재개 경로를 반드시 한 번 시험할 것 (30초).**
   로봇이 **주행 중일 때** 서버에서 `fake_send_event_ack.py` 를 한 번 쏜다.
   젯슨 터미널 2 에 이 줄이 떠야 정상이다:
   `[event_gate_node] 재개 요청(관제 확인 버튼)을 받았으나 이미 주행 중이다`
   주행 중이라 로봇은 아무 반응도 안 한다 — 안전하다.
   **이 줄이 안 뜨면 정지시켰을 때 재개가 안 된다.** 에러는 하나도 안 난다.
   원인과 대처: 설치가이드 10-1절 (DDS 매칭 실패, Nav2 재시작으로 해결).

5. **모터 폭주 시 `estop.sh` 로는 못 멈춘다.** 그건 "설계된 정상 정지"라 Nav2 목표만 취소한다.
