...
| 7 | 이벤트 정지/재개 판정 | ✅ |
| **8** | **실물 이식 + 튜닝** | 🟡 주행 검증 완료 · 이벤트 체인 일부 남음 (아래 2026-08-19 갱신) |
...
### 2026-08-19 갱신 — 실물 주행이 된다. 무엇이 막고 있었나

**결과부터**

| 항목 | 값 |
|---|---|
| 맵 | `shipyard_map_hall_v1` (반지름 0.75 m, 여유 0.30 m, 웨이포인트 12개) |
| 한 바퀴 | **31.7초** (33.0 / 30.0 / 32.0 실측) — 처음엔 94초였다 |
| 연속 주행 | 44바퀴 이상, `consecutive_fails` 0, 모터 폭주 0 |
| 사람이 앞을 막으면 | 정지 후 20~25초 안에 그 웨이포인트를 포기하고 다음으로 |
| AMCL | yaw 오차 4.9도 / 위치 0.08 m |
| 정합 | 97.5% |

**"기어가던" 원인은 하나가 아니라 셋이었다.** 셋 다 서로를 가리고 있어서
하나씩 고쳐도 증상이 안 사라졌다.

1. **감속 구간을 영원히 벗어나지 못했다** — `approach_velocity_scaling_dist`
   0.4 m 인데 웨이포인트 간격이 0.39 m 였다. 출발하자마자 "도착 직전"이라
   최저속 0.05 m/s 로 기어갔다. -> 0.15 로.
2. **heading_hold 가 안 꺼져 있었다** — Nav2 시작 시 끄는 `ros2 param set` 이
   조용히 실패했다(디스커버리 전 실행 -> exit 1, 로그에 묻힘). 그동안
   `wheel_odom_bridge` 가 Nav2 의 조향을 통째로 버리고 자기 값으로 덮어썼다.
   -> 성공할 때까지 재시도하는 헬퍼로 교체.
3. **제자리 회전을 시작하지 못했다** — RPP 는 목표 각속도를 **측정 각속도**
   기준으로 한 주기씩만 올린다. `max_angular_accel` 1.2 / 20 Hz 이면 첫 명령이
   0.06 rad/s 인데, 바퀴 속도로 초당 9 mm 라 모터 사구간을 못 넘는다.
   -> 안 돎 -> 측정 0 -> 또 0.06. 영원한 데드락. -> 6.0 으로.

**그리고 위를 다 고쳐도 로봇은 안 움직였다.** 폭주 방지 잠금이 걸려 있었고,
`wheel_odom_bridge` 는 `localization.launch.py` 소속이라 **Nav2 를 껐다 켜도
안 풀린다.** 상태 확인 겸 해제:

    ros2 service call /wheel_odom_bridge/reset_runaway std_srvs/srv/Trigger
    # 응답 '폭주 잠금 해제' = 잠겨 있었음 / '잠겨 있지 않았음' = 아니었음

**비상정지 스크립트도 세 겹으로 고장나 있었다** — `set -u` 때문에 ROS 환경
소싱에서 조용히 죽어 **신호를 한 번도 발행한 적이 없었고**, 고친 뒤에도
0.6초 만에 다 쏘고 끝나 디스커버리를 못 기다렸으며, 확인 루프도 두 번 틀렸다.
지금은 `estop.sh` / `resume.sh` 왕복이 `rc=0` 으로 통과한다.

상세와 재발 방지는 **설치가이드 10장 "아무 에러 없이 조용히 실패하는 것들"**
에 모았다. 오늘 겪은 실패는 대부분 **에러가 안 뜨는 종류**였다.

**아직 남은 것**

| 항목 | 상태 |
|---|---|
| 서버 → 젯슨 재개 경로 | 🟡 동작하지만 **DDS 매칭이 한 번 조용히 깨졌었다.** 코드 수정 없이 Nav2 재시작으로 복구됐다. 원인 미확정(CPU 포화 추정). **시연 전 30초 점검 필수** — 설치가이드 10-1 |
| YOLO + 영상송출 + Nav2 동시 구동 | 🟡 실측 합계 약 3.7~4.0 / 6코어. 가능하지만 빠듯하다. YOLO 추론 10 Hz -> 4~5 Hz 로 내리면 0.5~0.7코어가 생긴다 |
| `localization.launch.py` 의 YOLO 중복 | ⏳ systemd `yolo-depth-publisher.service` 와 겹쳐 **런치 쪽이 매번 조용히 죽는다.** 카메라를 한 프로세스만 열 수 있기 때문. 담당자와 소유권 정리 필요 |
| 전체 이벤트 체인 (YOLO 감지 -> 정지 -> 서버 확인 -> 재개) | ⏳ 각 구간은 개별 검증됨. 끝에서 끝까지는 아직 |
