...
#### ⚠️ `ack_cooldown_s` 가 필요한 이유

```
불 감지 -> 정지 -> 관리자 확인 -> 재개 -> 출발
  -> 불이 아직 그 자리에 있음 -> 트래킹 끊겼다 재획득 -> 새 track ID -> 또 정지!
```

`reported_tids`는 같은 추적 ID만 막으므로, 트래킹이 끊기면 같은 물체가
새 ID로 다시 보고된다. **확인을 눌렀는데 몇 초 뒤 또 멈추는 루프**가 생긴다.
확인 직후 일정 시간 감지를 무시해 그 구역을 벗어날 시간을 준다.
