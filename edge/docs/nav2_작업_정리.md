...
**2026-08-14 보강:** 같은 자리·같은 방향에서 해도 theta 는 **세션마다 새로 뽑힌다.**
UWB 측정 노이즈가 매번 다르기 때문이다. 즉 테이프는 재현성을 "높이는" 것이지
보장하는 것이 아니다. 실무적으로는 **맵을 재사용하기 전에 RViz 에서
`/scan_filtered` 가 맵 벽선과 겹치는지 눈으로 확인**하는 것이 유일하게
확실한 검증이다. 어긋나 있으면 재매핑한다.

`min_travel` 을 1.0 → 1.4 m 로 올린 것도 이 오차를 줄이기 위한 조치다
(PR #19). 주행 명령 1.5 m 와 판정 1.4 m 는 **마진을 둔 한 쌍**이라 한쪽만
바꾸면 타임아웃이 난다.

### 🔴 이슈 2-1. 모형 배는 라이다에 잡히지 않는다

순찰 중심 자동 탐지와 코스트맵 장애물 인식이 모두 실패한다.
매핑 시 임시 상자로 우회하고, 뎁스 측량 + KeepoutFilter 로 대체할 계획이다.
→ 상세는 **4-1장**, 젯슨 측 요청서는 [../../docs/배_중심좌표_측량_요청.md](../../docs/배_중심좌표_측량_요청.md)
...
## 4-1. 배 중심 순찰 설계 (2026-08-14)

지금까지 순찰 중심을 **맵에서 자동으로** 찾아 왔다. 모형 배가 들어오면
이 방식이 통하지 않는다. 그 이유와 대응을 정리한다.

### 문제 ① 배가 라이다에 안 잡힌다

라이다는 지면 약 0.20 m 높이의 **수평면 한 장**만 스캔한다. 모형 배는
전체 높이가 그 평면을 넘더라도 그 높이에 걸리는 것은 **마스트뿐이고
선체는 훨씬 아래**에 있다. 마스트는 폭이 몇 cm라 스캔점이 한 바퀴에
한두 개 잡히는 수준이라 노이즈와 구분되지 않는다.

배를 통째로 높이는 것은 답이 아니다. **뎁스카메라가 갑판 위 결함을 봐야
해서** 배 높이는 카메라 높이에 맞춰 이미 결정돼 있다.

결과가 두 가지인데, 두 번째가 더 위험하다.

| | 결과 |
|---|---|
| 맵에 배가 없음 | 순찰 중심 자동 탐지 실패 |
| **코스트맵에 배가 없음** | **Nav2가 빈 공간으로 알고 가로지를 수 있음** |

두 번째가 위험한 이유는 코스트맵의 장애물 입력이 라이다뿐이기 때문이다
(`observation_sources: scan`). 뎁스카메라는 코스트맵 입력이 아니다.
**중심 좌표를 안다고 해서 장애물로 인식되는 것이 아니다.**

### 문제 ② 자동 탐지는 "어느 것"을 구분하지 못한다

`check_patrol_space.py` 의 규칙은 이렇다.

1. 맵 테두리에서 flood fill → **바깥과 이어진 장애물(벽, 벽에 붙은 가구)을 제외**
2. 남은 것 = 자유공간에 완전히 둘러싸인 섬
3. 여러 개면 **픽셀 수가 가장 큰 것**을 선택

대부분은 후보가 하나라 잘 맞는다. 방 한가운데 떠 있는 물건이 둘 이상일 때만
문제가 되고, 그때 크기로 고른다.
...
배 자리에 0.25 m 이상 높이의 종이상자를 놓고 매핑한 뒤 치운다.
맵에 선체 윤곽이 남아 자동 탐지가 되고, `static_layer` 가 그 자리를
장애물로 들고 있어 글로벌 플래너가 배를 피한다. 코드 변경 0.
절차는 `재매핑_체크리스트.md` 0단계 참조.

한계: `static_layer` 는 **글로벌 코스트맵에만** 있다. 로컬 코스트맵은
`["obstacle_layer", "inflation_layer"]` 뿐이라 컨트롤러 단에서는
...
매핑 랩 1회
  ├─ SLAM                → 맵
  └─ ship_survey_node    → 배 표면점 누적 → 중심·yaw
       (젯슨 담당. docs/배_중심좌표_측량_요청.md 로 요청함)
                ↓  /ship_survey/pose (1회, TRANSIENT_LOCAL)
                ↓  /tmp/ship_survey_results/ship_pose_*.json
finalize_map.py
  ├─ config/patrol_<맵>.yaml        중심·반지름
  └─ masks/keepout_<맵>.pgm/.yaml   마스크 이미지
                ↓
navigation.launch.py
  ├─ filter_mask_server          (map_server 두 번째 인스턴스)
  ├─ costmap_filter_info_server
  └─ KeepoutFilter               글로벌 + 로컬 양쪽에 적용
```

`nav2_costmap_2d::KeepoutFilter` 는 Humble 에 이미 포함돼 있다.
센서와 무관하게 코스트맵에 직접 영역을 박는 방식이라 **라이다가 못 봐도,
스캔 레이트레이싱에 지워지지도 않는다.** 양쪽 코스트맵에 붙일 수 있어
1단계의 한계(로컬 미보호)가 해소된다.

설계상 주의:

- **마스크는 매핑 시점에 파일로 굽는다.** 런타임 생성 노드를 두지 않는다 —
  실행 중에 실패할 지점을 만들지 않기 위해서다. 세션 도중 배가 움직이는
  경우(`interface.md` ④의 "조립 단계 변경")에만 `map_server` 의 `LoadMap`
  서비스로 교체하면 되고, 그것은 나중에 얹는다.
- **측량은 매핑과 같은 주행에서 한다.** Nav2를 켠 뒤에 측량하면 순서가
  꼬인다 — 순찰을 돌려면 중심이 이미 있어야 하는데 중심을 구하려고 도는 것이라
  닭-달걀이 된다.
...
없다. 새 노드·서버는 전부 기존 launch 파일 **안쪽**에 들어가고,
`finalize_map.py` 의 측량 기록 경로도 `--align-dir`·`--calib-dir` 처럼
기본값을 두므로 사용자가 치는 명령줄은 그대로다.
...
#### ✅ 종결됨 — `vy` 미융합은 시뮬 전용 문제였다 (2026-08-07 실물 검증)

**처음 판단(오류).** 시뮬에서 로봇이 정지 상태인데도 `odom` 프레임이 옆으로
흘러 `odom->base_link`의 y가 −3 m까지 밀렸다. 원인을 `ekf_local.yaml`의
`odom0_config`에서 `vy`가 `false`인 것으로 보고, **실물에도 같은 문제가
있을 것**이라 판단해 슬램 담당자에게 수정을 요청하려 했다.

| | 값 |
|---|---|
| 입력 `/wheel/odom` 의 `vy` | 0.0 |
| 출력 `/odometry/local` 의 `vy` | **−0.043 m/s** (정지 상태인데도) |
| 결과 | `odom->base_link` 의 y가 −3 m |

**실물 검증 결과 — 문제 없음.**

| 측정 | 결과 |
|---|---|
| 정지 1~2분 | `odom->base_link` = (0.000, 0.000). 전혀 안 밀림 |
| 제자리 360° 회전 ×2 | 한 바퀴당 **6~8 mm** 누적 |

한 바퀴 돌면 제자리로 돌아오므로 남은 값이 곧 오차인데, 바퀴 오도메트리로는
**우수한 수준**이다. 시뮬의 3 m와는 자릿수가 다르다.

**왜 달랐나 (초기 설명의 오류 정정).**
프로세스 노이즈는 **불확실성(분산)** 을 키우지 추정값(평균)을 움직이지 않는다.
`vy` 추정값이 0에서 벗어나려면 다른 상태와의 **상관을 통해 밀려야** 하고,
그 상관은 로봇이 **실제로 회전할 때** 생긴다.

- **시뮬**: 접촉 불안정으로 계속 미세하게 흔들림(초당 1.6 mm + 롤 0.02 rad/s).
  이 지속적인 회전 성분이 `vy` 상태를 계속 여기시켰다.
- **실물**: 바닥에 안정적으로 정지. 여기 요인이 없어 `vy`가 0에 머문다.

**→ Gazebo 물리 아티팩트였고 `ekf_local.yaml` 설정 자체의 결함이 아니다.**

**조치 (확정).**

| 대상 | 조치 |
|---|---|
| 실물 `ekf_local.yaml` · `wheel_odom_node.py` | **수정하지 않음.** 슬램 담당자에게 요청도 하지 않음 |
| 시뮬 `sim_bringup.launch.py` 의 `odom0_config` 오버라이드 | **유지.** 시뮬 물리 아티팩트를 막는 용도 |

> **참고로 남겨두는 사실:** `wheel_odom_node.py`의 `twist.covariance[7]`(vy 분산)은
> 대입되지 않아 0.0이다. 지금은 `vy`를 fuse하지 않으므로 무해하다.
...
| 상황 | 수정 전 | 수정 후 |
|---|---|---|
| 정지 시 `odom->base_link` | y = −3.077 m | **y = 0.000 m** |
| 정지 시 map→base 위치 오차 | 140 mm | **2 mm** |
| 제자리 회전 중 위치 오차 | — | **0.1 mm** |
| 회전 중 각도 오차 | 1~5° (톱니) | 3.4° @0.6 rad/s |

**남은 각도 오차 3.4°는 `transform_tolerance`(0.03초)에 의한 구조적 지연**이다.
순찰 속도(회전 약 0.16 rad/s)로 환산하면 0.9° 수준이라 실사용에 지장이 없다.

함께 고친 것 두 가지 (둘 다 `fake_global_localization.py`):
- **콜백 안에서 TF를 블로킹 대기하지 않는다.** 단일 스레드 실행기에서
  기다리면 그동안 `/tf` 수신 콜백이 못 돌아 오히려 조회 성공률이 80 %로 떨어진다.
- **참값과 `odom->base_link`를 같은 시각으로 맞춰 합성한다.** 시각이 다르면
  그 사이 이동량이 그대로 오차가 된다(AMCL 등 표준 노드가 쓰는 방식).
