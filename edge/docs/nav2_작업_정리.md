...
## 0-1. ★★★ 반드시 알아야 할 것 — heading_hold 와 Nav2 는 같은 바퀴를 두고 싸운다

**이 문서에서 제일 중요한 한 절이다. Nav2 를 건드리는 사람은 여기부터 읽을 것.**

### 증상

로봇이 순찰 중 **덜덜 떨면서** 간다. 복구행동(spin/backup/wait)이 잦아진다.
어떤 날은 멀쩡하고 어떤 날은 심하다 — 재현이 들쭉날쭉해서 "PID 가 거칠다",
"슬립 때문이다" 같은 엉뚱한 진단으로 새기 쉽다.

### 원인

`wheel_odom_bridge` 의 `heading_hold` 는 **"직진 의도(|w| < 0.02)"** 로 보이는
`/cmd_vel` 이 오면 자체 yaw 보정(kp 1.0, ki 0.5)을 얹는다. teleop·캘리브레이션
직진에는 꼭 필요한 기능이다.

그런데 **Nav2 가 조향하는 동안 이게 켜져 있으면 두 컨트롤러가 같은 바퀴를
두고 싸운다.** 순찰 웨이포인트 사이 구간은 거의 직선이라 w 가 수시로 0 근처로
떨어지므로, heading_hold 가 끼어들었다 빠졌다 하며 로봇이 떤다.

### 예전 구조의 결함 (2026-08-27 발견)

Nav2 launch 가 `ExecuteProcess` 로 `ros2 param set ... false` 를 **노드들과 함께**
띄웠다. 그런데 `ros2 param set` 은 매번 새 ROS 노드를 만들어 DDS 디스커버리를
처음부터 하므로 느리다. 런치 로그 전수 조사 결과:

```
성공                     75회
성공 못 하고 죽음         12회   ← 이 경우 heading_hold 가 켜진 채로 순찰
성공까지 걸린 시간        9~11초  (측정한 3회 모두)
```

그리고 `patrol_mission_node` 는 지연 없이(`create_timer(0.2, self._tick)`)
목표를 쏘기 시작한다. **즉 Nav2 를 켠 뒤 약 10초 동안 두 컨트롤러가 동시에
조향할 수 있었다.** "어떤 날은 멀쩡하고 어떤 날은 떤다" 의 정체가 이 경쟁이다.

### 현재 구조 (2026-08-27 수정)

순서를 뒤집었다. `navigation.launch.py` 의 `generate_launch_description()` 첫 줄에서
`_wait_heading_hold_off()` 가 **동기적으로** 돈다:

```
⏳  heading_hold 가 꺼지길 기다립니다 — 아직 로봇은 움직이지 않습니다
        ↓  (꺼질 때까지 여기서 대기, 다른 배너는 하나도 안 올라온다)
✅  heading_hold 를 껐습니다 (N번째 시도, X초)
    이제 Nav2 를 띄우고 로봇이 움직이기 시작합니다.
```

- 꺼진 것을 확인하기 전에는 **노드가 하나도 안 뜬다.** 로봇이 움직일 방법
  자체가 없으므로 경쟁이 구조적으로 사라진다.
- 60초 안에 못 끄면 **Nav2 를 아예 시작하지 않는다**(`sys.exit(1)`). 이 상태로
  순찰하면 주행이 망가지므로, 조용히 진행하는 것보다 안 뜨는 편이 낫다.
  대개 로컬라이제이션이 안 떠 있는 경우다.
- 종료 시 `true` 로 되돌리는 것은 그대로 둔다 (teleop·캘리브레이션용).

### 건드릴 때 지킬 것

- **비동기로(`ExecuteProcess`) 끄지 말 것.** 그게 이 사고의 원인이었다.
  런치 전에 동기적으로 끝내야 한다.
- `heading_hold_w_threshold`(0.02)를 키우면 더 자주 개입하게 되어 더 나빠진다.
- 증상이 "떨림" 이라고 바퀴 PID(아두이노)나 RPP 파라미터부터 만지지 말 것.
  **먼저 heading_hold 가 실제로 꺼졌는지 확인한다:**

  ```bash
  ros2 param get /wheel_odom_bridge enable_heading_hold   # false 여야 정상
  ```

---

## 0-2. 조립 단계(공정률) 판정 정책 — 잘린 배로는 판정하지 않는다

**결정 (2026-08-27):** `reject_level_touching_edge=True` 를 **실물에서도 유지한다.**

### 근거

조립 단계는 **위험 이벤트가 아니다.** 불이나 쓰러진 사람은 늦게 알면 사고가
되지만, 공정률은 한 바퀴에 한 번만 맞게 갱신돼도 충분하다.

실측으로 배가 잘리면 판정이 뒤집히는 것이 확인됐다:
...
그래서 **갱신 빈도를 내주고 정확도를 산다.** 순찰 궤도 어딘가에는 배가
온전히 들어오는 지점이 생기므로, 그 지점의 판정만 받으면 된다.
위험 이벤트였다면 반대로 골랐겠지만(늦는 것보다 틀려도 알리는 편이 낫다),
조립 단계는 그래도 된다.

### 이 정책이 만드는 위험과 방어

배가 **한 번도** 온전히 안 잡히면 공정률이 **조용히 멈춘다.** 에러가 아니라
그냥 조용한 거라 눈치채기 어렵다 — 2026-08-27 하루 동안 우리를 가장 많이
괴롭힌 실패 유형이 정확히 이것이었다(change_point 가 죽어도 팝업·핑은
계속 떠서 멀쩡해 보였다).

그래서 `_warn_if_level_starved()` 가 2분 이상 온전한 배를 못 보면 경고한다:

```
[조립단계] 5분째 배가 화면에 온전히 안 담긴다 (잘려서 제외한 검출 312건).
           공정률이 갱신되지 않고 있다 — 순찰 반경을 넓히거나,
           reject_level_touching_edge 를 False 로 두고 부분 뷰 학습 모델을 쓸 것
```

이 경고가 계속 뜨면 둘 중 하나를 골라야 한다:
1. **순찰 반경을 넓힌다** — 배가 화면에 다 들어오는 지점이 생기게
2. **부분 뷰로 학습된 모델을 쓰고** `reject_level_touching_edge=False` 로 바꾼다

2번이 실물 대형 선박의 최종 해답이다. 순찰 거리에서 배 전체를 담으려면
너무 멀어져서 순찰의 의미가 없어지기 때문이다. 다만 **그건 학습의 문제이지
알고리즘의 문제가 아니므로**, 모델이 준비되기 전까지는 1번 정책으로 간다.
...
### 대상이 맵에 없으면 순찰 반지름을 크기로 정한다 (2026-08-14)

라이다에 안 잡히는 대상은 맵이 비어 있어 `check_circle` 이 "아무 반지름이나
통과한다"고 판정한다. 그러면 **대상 안쪽을 도는 원**이 뽑힌다.

```
0.787 x 0.181 m 배 → 반지름 0.20 m 선택
                     웨이포인트 12/12 가 keepout 금지영역 안 = 배 속을 도는 설정
```

keepout 을 만든 이유가 이 사고인데 순찰 설정 쪽에 같은 구멍이 남아 있었다.
`mask_size` 를 아는 경우에는 차체 안쪽 끝(`sweep_bounds(r)[0]`)이
`hypot(sx/2, sy/2) + margin` 을 넘도록 최소 반지름을 먼저 정하고 그 아래는
후보에서 뺀다. 위 예에서 0.20 → **0.60 m** 로 바뀐다.

### 뱃머리 방향은 운영 규칙으로 해소한다

`cv2.minAreaRect` 는 직사각형만 주므로 뱃머리와 선미를 구분하지 못한다
(180도 모호성). **Nav2 는 이 모호성이 무해하다** — 금지 영역도 순찰 원도
180도 대칭이라 결과가 같다. 프론트 3D 모델만 뒤집혀 보인다.

코드로 푸는 대신 **"배는 항상 map `+x` 방향으로 뱃머리가 가게 놓는다"**는
운영 규칙으로 해결했다 (`interface.md` ④). 젯슨의 `ship_survey_node` 는
이 규칙을 전제로 `yaw_hint_rad=0.0` 기본값을 쓴다. YOLO 에 뱃머리 클래스를
추가하는 방안은 학습 비용 대비 이득이 없어 채택하지 않았다.
