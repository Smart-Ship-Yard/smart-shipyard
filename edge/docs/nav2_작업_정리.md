...
#### 🔴 Step 3에서 발견한 실물 영향 버그 — `ekf_local.yaml`의 `vy` 미융합

**증상 (시뮬 실측 2026-08-07).** 로봇이 정지해 있는데도 `odom` 프레임이 옆으로
계속 흘러 `odom->base_link`의 y가 **−3 m까지 밀렸다.** RViz에서는 로봇 모델과
라이다 스캔이 멀찍이 떨어져 보이고 스캔이 로봇을 따라 도는 것으로 나타난다.

**원인.** `ekf_local.yaml`의 `odom0_config`에서 **`vy`가 `false`**다.

```yaml
odom0_config: [..., true,  false, false,   # vx, vy, vz   <- vy가 false
```

차동구동 로봇은 옆으로 이동할 수 없으므로(비홀로노믹 제약) `vy`는 항상 0이다.
이를 필터에 알려주지 않으면 프로세스 노이즈가 `vy` 상태를 흔들고 그것이 계속
적분되어 위치가 옆으로 무한정 흘러간다. robot_localization의 대표적인 함정이다.

| | 값 |
|---|---|
| 입력 `/wheel/odom` 의 `vy` | **0.0** (정상) |
| 출력 `/odometry/local` 의 `vy` | **−0.043 m/s** (정지 상태인데도) |

**⚠️ 시뮬만의 문제가 아니다.** `wheel_odom_bridge`도 `vy: 0`을 보내므로
**실물에서도 동일하게 발생한다.** 실물은 `ekf_global`의 UWB가 최종 위치를
잡아주지만, `odom->base_link`가 계속 밀리면 Nav2 로컬 코스트맵이 흔들린다.

**대응 (C안 채택).**

| | 조치 | 상태 |
|---|---|---|
| 시뮬 | `sim_bringup.launch.py`에서 파라미터 오버라이드 (`vy`를 `true`로) | ✅ 적용 |
| 실물 | 슬램 담당자에게 `ekf_local.yaml` 수정 요청 | ⏳ 확인 대기 |

원본이 수정되면 **launch의 오버라이드를 제거한다.** 슬램 담당자 파일 불가침
원칙을 지키기 위해 우리 쪽에서는 덮어쓰기만 했다.

> PR #14는 같은 파일의 `imu0_config`(약 10행 아래)를 수정한다.
> `odom0_config`와는 떨어져 있어 **git 자동 병합이 되며 충돌하지 않는다.**
