...
```
yolo_depth_publisher
        │
        ├──> /event_detection/uvd      (엣지 트리거, 객체당 1번)
        │        └──> websocket_client.py ──> 서버   (기존, 미수정)
        │
        └──> /event_detection/present  ★ 신규 요청 (매 프레임, 현재 보이는 클래스 목록)
                 └──> event_gate_node.py ──> /event/active (Bool)
                          └──> patrol_mission_node
```

```yaml
event_gate_node:
  present_topic: /event_detection/present
  trigger_classes: [fire, fallen_person, no_helmet]   # ship_defect 제외 확정
  detect_hold_s:   1.0    # 1초 연속 감지 -> 정지 (안전은 빨라야 함)
  clear_hold_s:    7.0    # 7초 연속 빈 목록 -> 재개
  signal_timeout_s: 5.0   # 이 시간 아무 메시지도 없으면 비전 이상으로 판단
  on_signal_loss: stop    # stop | continue (리허설에서 결정)
```
...
**동시 다발 이벤트:** 타이머를 클래스별로 두지 않고 **목록 전체가 빌 때만** 센다.
화재와 낙상이 같이 잡혔다가 화재만 해결되면 목록에 낙상이 남으므로 계속 정지한다.

#### 신호 두절 감시 (`signal_timeout_s`) — 우리 쪽에서만 구현

`present` 토픽이 일정 시간 아무것도 안 오면 **비전 파이프라인 이상**으로 보고 정지한다.
시간을 재는 것은 받는 쪽만 할 수 있으므로 **비전 담당자 작업은 없다.**

끊긴 것은 비전이지 주행이 아니다. Nav2의 장애물 회피는 라이다로 하므로
비전이 죽어도 충돌하지 않는다. 그럼에도 정지시키는 이유는 **고장을 즉시 드러내기
위해서**다. 계속 순찰하면 소품을 놔도 반응이 없어 **비전 고장을 Nav2 고장으로
오해받는다.** 다만 시연 흐름을 우선한다면 `continue`가 나을 수 있어 파라미터로 뺀다.

**타임아웃을 5초로 넉넉히 잡은 이유:** 젯슨이 YOLO + Nav2 + EKF×2를 동시에 돌리므로
GPU가 밀려 1~2초 끊기는 것은 정상 범위다. 2초로 잡으면 멀쩡한데 발표 중 멈춘다.

#### ⏱ 실측 후 확정할 값 (Step 7 착수 시 반드시 수행)

`inference_interval_s: 0.1`(10 Hz)은 **타이머 설정값이지 실제 달성 주기가 아니다.**
콜백이 0.1초 안에 안 끝나면 밀린다. 게다가 **Nav2를 얹으면 부하가 더 늘어**
추론 주기가 느려질 수 있다. 전부 연결한 뒤 실측할 것:

```bash
ros2 topic hz /event_detection/present
```

| 실측 주기 | 조치 |
|---|---|
| 5 Hz 이상 | 위 기본값 그대로 사용 |
| 2~5 Hz | 사용 가능. `signal_timeout_s`를 여유 있게 확인 |
| 2 Hz 미만 | `detect_hold_s` / `signal_timeout_s` 재조정 필요 |

**값 확정은 리허설에서:** 소품을 정리하고 카메라 프레임 밖으로 나가기까지
스톱워치로 재고 +3초를 `clear_hold_s`로 잡는다. 너무 짧으면(사람이 아직 옆에 있는데
출발) 고장처럼 보이므로, 애매하면 긴 쪽을 택한다.

### 🟡 미결 — "이벤트 해결"의 판정 주체 (Step 7 착수 전까지 결정)

| 주장 | 방식 | 장점 | 단점 |
|---|---|---|---|
| Nav2 담당 | **비전이 clear 판정** (불 치움/헬멧 착용/사람 일어남) | 자체 완결, 시연 흐름이 안 끊김 | 가림에 의한 오판 가능 |
| 슬램·비전 담당 | **프론트 "확인" 버튼** | 실제 안전 운영에 부합 | 백엔드 수신 경로가 아직 없음, 발표 중 화면 전환 필요 |

**이 논쟁은 Step 3~6을 막지 않는다.** `/event/active` (Bool) 경계가 방화벽 역할을
하기 때문이다. `patrol_mission_node`는 "true면 정지, false면 주행"만 알 뿐
그 값을 누가 어떻게 정했는지 모른다. **영향받는 것은 Step 7뿐이다.**

**그리고 택일할 필요도 없다.** `event_gate_node`가 두 입력을 모두 받아
OR로 처리하면 된다. 지금은 비전만 연결하고, 백엔드 수신이 준비되면
입력 하나를 추가하는 것으로 끝난다(설계 변경 불필요).

**현재 방침:** 프로토타입 단계이고 백엔드 수신 경로가 미구현이므로 **비전 방식으로
진행**한다. 클래스별로 정책을 나누지 않는다(시간 대비 효용이 낮음). 세 클래스 모두
동일하게 정지시키고, 리허설에서 `no_helmet`이 과하다고 판단되면
`trigger_classes`에서 한 단어를 제거한다.
...
- 발행 주기는 **"추론할 때마다"**로 요청한다. 특정 Hz를 못박으면 별도 타이머와
  상태 변수가 필요해져 담당자 작업량이 늘어난다. 기존 `_process_frame` 안에서
  발행하면 한 줄로 끝나고 주기도 자동으로 맞는다.

**⚠️ 빈 목록 `[]`을 반드시 보내야 하는 이유 (요청서에서 가장 강조할 부분)**

이벤트가 없을 때 "아무것도 안 보내기"로 하면 아래가 전부 구별되지 않는다.

| 조용한 이유 | 로봇이 해야 할 일 |
|---|---|
| 이벤트 해결됨 | 재개 |
| 비전 노드 죽음 / 카메라 분리 / 프로세스 멈춤 | **정지 유지** |

`[]`는 빈 메시지가 아니라 **2바이트짜리 정상 페이로드**다(`json.loads("[]") -> []`).
따라서 `[]`가 계속 오는 것은 **"이벤트 없음" 신호인 동시에 "노드 살아있음"
하트비트**가 된다. 지금 `/event_detection/uvd`가 "조건 만족 시에만 발행"이라
문제가 생긴 것이므로, 같은 실수를 반복하지 않는 것이 핵심이다.
