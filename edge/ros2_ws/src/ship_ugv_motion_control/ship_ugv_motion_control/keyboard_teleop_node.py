...
class KeyboardTeleopNode(Node):
    ...
    def __init__(self):
        ...
        # ★ heading hold는 이제 wheel_odom_bridge가 담당한다 (2026-08 리팩토링).
        #   여기서는 w/s 입력 시 target_w를 그냥 0.0으로만 두면 되고,
        #   /odometry/local 구독이나 자체 보정 계산이 필요 없어졌다.
    ...
    def _publish_loop(self):
        ...
        with self._lock:
            ...
            v = self._current_v
            w = self._current_w
            # ★ w/s 입력 시 target_w=0.0 그대로 유지 -> 가속 램프를 거쳐도
            #   결국 w는 0에 수렴하고, wheel_odom_bridge가 이걸 감지해서
            #   heading_hold를 적용한다.
