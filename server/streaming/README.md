...
`mediamtx.yml` 에서 켜져 있다. **보관 48시간.**

| | |
|---|---|
| 저장 위치 | `~/mediamtx/recordings/ugv1/` (저장소 밖) |
| 형식 | fMP4, 1시간 단위 조각 |
| 용량 | 실측 1.2 Mbps → 시간당 약 540 MB · 하루 약 13 GB |
| 48시간 상주 | 약 **26 GB** |

> ### 왜 48시간인가
>
> 이 노트북 디스크 여유가 93 GB 다. 7일로 잡으면 91 GB 라 거의 다 먹는다.
> 2일이면 26 GB 로 넉넉하고, 어제 것까지 되짚어 볼 수 있어 관제 용도로는 충분하다.
>
